/* Generate sizeof(unsigned int) bytes of as random data as possible to seed
   the hash function.
*/
...
static unsigned int buf_to_uint32(char *data) {
    size_t i;
...
static int seed_from_urandom(unsigned int *seed) {
...
static int seed_from_windows_cryptoapi(unsigned int *seed)
{
...
static int seed_from_timestamp_and_pid(unsigned int *seed) {
...
volatile unsigned int hashtable_seed = 0;
